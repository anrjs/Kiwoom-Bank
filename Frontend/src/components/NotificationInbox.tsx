...
import { useEffect, useRef, useState } from "react"; // :느낌표:️ 제거
...
// import axios from "axios"; // :느낌표:️ 제거
// :느낌표:️ API_BASE, WATCHLIST, POLL_MS 제거
...
interface NotificationInboxProps {
  notifications: Notification[]; // :느낌표:️ App.tsx로부터 이 배열을 받음
  onBack: () => void;
  onHome: () => void;
  onSelectNotification: (id: number) => void; // :느낌표:️ App.tsx의 상태 변경 함수
}
export function NotificationInbox({
  notifications, // :느낌표:️ props로 받음
  onBack,
  onHome,
  onSelectNotification,
}: NotificationInboxProps) {
  // :느낌표:️ localNotifs, prevGradesRef, getRatingOrder, fetchGrades 및 모든 useEffect 폴링 로직 제거
  // ====== 클릭 시 부모(App.tsx)의 함수 호출 ======
  const handleSelect = (id: number) => {
    // :느낌표:️ 로컬 상태 변경 대신, 부모의 함수를 호출
  ...
  // :느낌표:️ UI는 props로 받은 notifications를 직접 사용
  // const list = localNotifs.length ? localNotifs : notifications; // :느낌표:️ 제거
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {/* Summary (:느낌표:️ list -> notifications로 변경) */}
        ...
        {/* Notifications List (:느낌표:️ list -> notifications로 변경) */}
        <div className="space-y-4">
          {notifications.map((notification, index) => (
            <Card
              key={notification.id}
              className={getCardStyle(notification.type, notification.isRead)}
              onClick={() => handleSelect(notification.id)} // :느낌표:️ 수정된 핸들러
            >
              <div className="p-5">
