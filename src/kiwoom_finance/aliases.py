...
KOR_KEY_ALIASES = {
    # ===== BS =====
    "current_assets": ["유동자산"],
    "current_liabilities": ["유동부채","유동부채총계","단기부채","유동부채합계"],
    "noncurrent_liabilities": ["비유동부채"],
    "total_liabilities": ["부채총계", "부채 총계"],
    "equity_total": ["자본총계", "자본 총계"],
    "equity_parent": ["지배기업소유주지분", "지배기업 소유주지분"],
    "equity_nci": ["비지배지분"],
    "total_assets": ["자산총계", "자산 총계", "부채와자본총계"],

    # ===== IS / CIS =====
    # 수익/매출: CIS에서 '영업수익', '수익' 등만 제공되는 경우 존재
    "revenue": [
        "매출액", "영업수익", "수익", "수익(매출액)", "매출", "매출액(수익)"
    ],
    # 영업이익: CIS에 없을 수도 있음(그 경우 operating_margin은 NaN이 정상)
    "operating_income": [
    ...
    # (선택) 총포괄손익: metrics에서 net_income이 끝내 없을 때 최후 폴백으로 사용
    "tci_total": [
...
# ---- BS 명칭 변형 보강 ----
KOR_KEY_ALIASES.update({
    "current_assets": KOR_KEY_ALIASES.get("current_assets", []) + [
        "유동자산총계","유동자산 총계","유동자산총액","유동자산 합계","유동자산계"
    ],
    "current_liabilities": KOR_KEY_ALIASES.get("current_liabilities", []) + [
        "유동부채총계","유동부채 총계","유동부채총액","유동부채 합계","유동부채계","단기부채총계","단기부채 총계"
    ],
    "total_assets": KOR_KEY_ALIASES.get("total_assets", []) + [
        "자산총계","자산 총계","자산총액","자산 합계","자산계",
        "부채와자본총계","부채와 자본 총계","자산과부채총계"
    ],
    "total_liabilities": KOR_KEY_ALIASES.get("total_liabilities", []) + [
        "부채총계","부채 총계","부채총액","부채 합계","부채계"
    ],
    "equity_total": KOR_KEY_ALIASES.get("equity_total", []) + [
        "자본총계","자본 총계","자본총액","자본 합계","자본계","자본과부채총계"  # 일부 보고서 오표기 방지
    ],
})
