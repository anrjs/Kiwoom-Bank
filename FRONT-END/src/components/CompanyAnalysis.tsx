...
export function CompanyAnalysis({ companyName, onBack }: CompanyAnalysisProps) {
  ...
  const fetchRating = async () => {
    ...
    try {
      ...
      if (response.data && response.data.predicted_grade) {
        const newRating = response.data.predicted_grade;
        
        // 이전 등급이 있고 새로운 등급과 다르면 변화 감지
        if (previousRating && previousRating !== newRating) {
          setRatingChange(previousRating + " → " + newRating);
        ...
        setPreviousRating(aciRating !== "예측 중..." ? aciRating : "");
  ...
  // ✅ 실시간 새로고침 (10분마다 등급 업데이트)
  useEffect(() => {
    const interval = setInterval(() => {
      fetchRating();
    }, 600000); // 10분(600,000ms)마다 호출
  ...
  const getRatingChangeIcon = (change: string) => {
    if (change.includes("→")) {
      ...
      const prevOrder = getRatingOrder(prev.trim());
      const nextOrder = getRatingOrder(next.trim());
      
      if (nextOrder < prevOrder) {
        return <TrendingUp className="w-5 h-5" />; // 등급 상승
      } else if (nextOrder > prevOrder) {
        return <TrendingDown className="w-5 h-5" />; // 등급 하락
      }
    }
  ...
  const getRatingChangeColor = (change: string) => {
    if (change.includes("→")) {
      ...
      if (nextOrder < prevOrder) {
        return "text-green-600"; // 등급 상승 (녹색)
      } else if (nextOrder > prevOrder) {
        return "text-red-600"; // 등급 하락 (빨간색)
      }
    }
